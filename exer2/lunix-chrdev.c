...
static int lunix_chrdev_state_update(struct lunix_chrdev_state_struct *state)
{
    ...
    switch (state->type) {
        case 0 : 
			int_info = lookup_voltage[raw_value];
			break;
        case 1 : 
			int_info = lookup_temperature[raw_value];
			break;
        case 2 : 
			int_info = lookup_light[raw_value];
			break; 
        default: 
...
static ssize_t lunix_chrdev_read(struct file *filp, char __user *usrbuf, size_t cnt, loff_t *f_pos)
{
	...
	/* ? */
    int not_copied, delta, to_copy = (*f_pos + cnt) < sizeof(text) ? cnt : (sizeof(text) - *f_pos);
