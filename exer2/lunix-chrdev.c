...
static int lunix_chrdev_state_update(struct lunix_chrdev_state_struct *state)
{
    ...
    msr_data = sensor->msr_data[state->type];
    uint16_t raw_value = msr_data->value[0];  //check for uint32_t
    ...
    long int_info;
    switch (state->type) {
        case 0 : int_info = lookup_voltage[raw_value];
        case 1 : int_info = lookup_temperature[raw_value];
        case 2 : int_info = lookup_light[raw_value]; 
        default: pr_error("invalid measurement type\n");
    }

    long integer_part = int_info / 1000;
    long decimal_part = int_info % 1000;
    char text_info[20];
    snprintf(text_info, sizeof(text_info), "%ld.%ld", integer_part, decimal_part);  //check int long types

    down_interruptible(&state->lock);
    for (int i=0; i<sizeof(text_info), i++) state->buf_data[i] = text_info[i];
...
static int lunix_chrdev_open(struct inode *inode, struct file *filp)
{
    ...
    private_state = kzalloc(sizeof(lunix_chrdev_state_struct), GFP_KERNEL);
...
static ssize_t lunix_chrdev_read(struct file *filp, char __user *usrbuf, size_t cnt, loff_t *f_pos)
{
	...
	if (*f_pos == 0) {
		while (lunix_chrdev_state_update(state) == -EAGAIN) {
			...
			/* See LDD3, page 153 for a hint */
             wait_event_interruptible(wq, state->buf_timestamp >= msr_data->last_update);
		}
	}
	...
	/* End of file */
	/* ? */
    if (*f_pos > sizeof(text)) {
        *f_pos = 0;
        return 0;
    }
	/* Determine the number of cached bytes to copy to userspace */
	/* ? */
    int not_copied, delta, to_copy = (*off + cnt) < sizeof(text) ? cnt : (sizeof(text) - *f_pos);
    not_copied = copy_to_user(usrbuf, &text, to_copy);
